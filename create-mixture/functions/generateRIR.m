function generateRIR(fs, roomDim, RT60, centerSensor, distSensors, distSources, azimuthAngle, elevationAngle,rirFolderName)
...
% rirFolderName = [rir_path 'RIR_dim_', roomDimStr, '_center_', num2str(centerSensor(1)), '_', num2str(centerSensor(2)), '_', num2str(centerSensor(3)), '_fs_', num2str(fs/1000), 'k/'];
% mkdir(rirFolderName);
    ...
    for azimuth = 1 : numAzimuth
        for elevation = 1 : numElevation
            ...
            for sensor = 1 : numSensor
                ...
                rirFileName = ['dist_' num2str(distSources(dist)), '_angle_', num2str(azimuthAngle(azimuth)),'_', num2str(elevationAngle(elevation)), '.mat'];
