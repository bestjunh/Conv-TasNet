function H = mixingNxN_woSNR_LateReverb(mixoutRoot, signalFileName, RT60, lateStartSec, distSensors, distSources, azimuthAngle, elevationAngle, targetFs, upFs, roomDim, centerSensor,rir_path)

% NxN mixing
disp(['RT60 = ',num2str(RT60),' (sec)']);
...
disp(['Number of Source = ' num2str(numSource), ', Number of Sensor = ' num2str(numSensor), ...
    ',  RT60 = ',num2str(RT60),' (sec)']);

% GENERATING ROOM IMPULSE RESPONSE
k = 0.161;
[H, locationSensor, locationSource] = genNxN(upFs, roomDim, RT60, k, centerSensor, distSensors, distSources, azimuthAngle, elevationAngle, 12, rir_path);

% % Align
% min_idx = length(H(:,1,1));
% for ch = 1:numSensor
%     for i = 1:numSource        
%         [~,idx] = max(H(:,ch,i));
%         if idx < min_idx
...
%         H_ = H(:,ch,i);
%         H(:,ch,i) = 0;
%         H(min_idx:min_idx+length(H_(idx:end))-1,ch,i) = H_(idx:end);
%     end
% end
% 

% Delete early and direct components
lateStartUpSample = lateStartSec*upFs;
for ch = 1:numSensor
    for i = 1:numSource
        [~,idx] = max(H(:,ch,i));
        H(1:idx+lateStartUpSample,ch,i)= 0;
    end    
end
...
s_energy = sum(s{1,1}.^2);
for j = 1 : numSource
    n_energy = sum(s{1,j}.^2);
    ratio = s_energy./n_energy;
    for i = 1 : numSensor
        s{i, j} = s{i, j} .*ratio;
    end
end
...
for i = 1: numSensor
    ...
    if i == 1
        ...
        for j = 1:numSource
            maxScale = max(maxScale, max(abs(cell2mat(s(i,j)))));
        end
    else
