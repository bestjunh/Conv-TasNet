...
% RIRPATH = ['/home/dev60-data-mount/albert/DB/CONV-TASNet-RIR-v2/noise/']
upFs = 3*16000;
    ...
    for roomDim_idx = 1:length(ROOMDIM)
        ...
        for locDelta_idx = 1:length(LOCDELTA)
            ...
            if 0
                ...
                hold on;
                for n = 1:Nd
                    v = Sphe2Cart(r(n),azi(n),ele(n));
                    plot3(centerSensors(1)+v(1),centerSensors(2)+v(2),centerSensors(3)+v(3), 'bo');
                end
                hold off;
                xlabel('x');
                ylabel('y');
